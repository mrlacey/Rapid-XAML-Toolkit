    ...
    internal class GetXamlFromCodeWindowBaseCommand : BaseCommand
    {
        ...
        public async Task<ParserOutput> GetXamlAsync(IAsyncServiceProvider serviceProvider)
        {
            ...
            if (settings.Profiles.Any())
            {
                ...
                {
                    SharedRapidXamlPackage.Logger?.RecordError(
                        "Failed to get DTE in GetXamlFromCodeWindowBaseCommand.GetXamlAsync",
                        force: settings.ExtendedOutputEnabled);
                }
                else
                {
        ...
        protected static async Task ShowStatusBarMessageAsync(IAsyncServiceProvider serviceProvider, string message)
        {
            try
            {
                ...
                {
                    SharedRapidXamlPackage.Logger?.RecordError(
                        "Failed to get DTE in GetXamlFromCodeWindowBaseCommand.ShowStatusBarMessageAsync",
                        force: CodeParserBase.GetSettings().ExtendedOutputEnabled);
                }
            }
            catch (Exception exc)
            {
