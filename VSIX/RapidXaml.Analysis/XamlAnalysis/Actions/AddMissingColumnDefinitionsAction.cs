...
namespace RapidXamlToolkit.XamlAnalysis.Actions
{
    public class AddMissingColumnDefinitionsAction : MissingDefinitionsAction
    {
        protected AddMissingColumnDefinitionsAction(string file)
            : base(file)
        {
            this.UndoOperationName = StringRes.UI_UndoContextAddMissingColumnDefinitions;
        ...
        public override void Execute(CancellationToken cancellationToken)
        {
            ...
            vs.StartSingleUndoOperation(StringRes.UI_UndoContextAddColumnDefinitions);
            ...
            try
            {
                if (this.Tag.UsesShortDefinitionSyntax)
                {
                    ...
                    while (true)
                    {
                        ...
                        if (line.GetText().Contains("ColumnDefinition=\""))
                        {
                            vs.ReplaceInActiveDocOnLine(
                                "ColumnDefinition=\"",
                                "ColumnDefinition=\"" + string.Concat(Enumerable.Repeat("*,", this.Tag.TotalDefsRequired - this.Tag.ExistingDefsCount + 1)),
                                lineNo);
                            break;
                        }
